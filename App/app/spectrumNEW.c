...
static bool InitScan() {
    ...
    if (appMode == SCAN_BAND_MODE) {
        // Если ни один бенд не включён — включаем первый доступный (иначе зависание)
        bool anyEnabled = false;
        for (uint8_t i = 0; i < bandCount; i++) {
            if (settings.bandEnabled[i]) { anyEnabled = true; break; }
        }
        if (!anyEnabled && bandCount > 0) {
            settings.bandEnabled[0] = true;
            nextBandToScanIndex = 0;
        }
    ...
    break;
        case KEY_UP:
            if (historyListActive) {
                ...
                // Проверяем есть ли включённые бенды
                uint8_t anyBand = 0;
                for (uint8_t _i = 0; _i < bandCount; _i++) if (settings.bandEnabled[_i]) { anyBand = 1; break; }
                if (anyBand) {
