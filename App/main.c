...
void Main(void)
{
    ...
    // USB активируется если нажата любая кнопка кроме PTT
    #ifdef ENABLE_USB
    const bool gKeyPressedAtBoot = (!GPIO_IsPttPressed() && KEYBOARD_Poll() != KEY_INVALID);
    #endif
    ...
    */
    ...
    {   // keys are pressed
        #ifdef ENABLE_USB
        if (gKeyPressedAtBoot) {
            // USB активирован — показываем "USB ACTIVATED"
    ...
    #ifdef ENABLE_USB
    // Отключаем USB клок только если кнопка НЕ была нажата при старте
    if (!gKeyPressedAtBoot) {
        LL_APB1_GRP1_DisableClock(LL_APB1_GRP1_PERIPH_USBD); // USB DEACTIVATED
    } else {
        VCP_Init(); // Запускаем USB стек
        AUDIO_PlayBeep(BEEP_880HZ_60MS_DOUBLE_BEEP); // USB activated beep
    }
    #endif
